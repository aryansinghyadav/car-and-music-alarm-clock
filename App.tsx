...
1400 call with Jerry`,
  ...
  const [viewMode, setViewMode] = useState<'clock' | 'vehicle'>('vehicle');
    ...
    <div className="min-h-screen flex items-center justify-center p-4 md:p-8 overflow-hidden select-none">
      ...
      <div className="relative bg-radio-case w-full max-w-4xl rounded-xl shadow-device border-t border-white/10 p-6 md:p-10 flex flex-col md:flex-row gap-8">
         ...
         <div className="flex-1 flex flex-col gap-6 relative z-10">
            ...
            {/* Top Display Panel */}
            <div className={`bg-radio-face border-4 border-radio-dim rounded-lg p-6 relative shadow-inset-screen overflow-hidden transition-all duration-500 flex flex-col ${viewMode === 'vehicle' ? 'min-h-[450px]' : 'min-h-[300px]'}`}>
                {/* Glass Glare */}
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-0 mb-4 relative z-10">
                    ...
                    <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-2">
                        <div className="flex gap-2">
                            <button 
                                onClick={() => setViewMode(prev => prev === 'clock' ? 'vehicle' : 'clock')}
                                className={`text-[10px] font-mono border px-2 py-0.5 rounded uppercase tracking-widest transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-radio-lit ${viewMode === 'vehicle' ? 'bg-radio-lit text-white border-radio-lit shadow-led' : 'text-gray-400 border-gray-600 hover:text-white'}`}
                            >
                                {viewMode === 'clock' ? 'Vehicle' : 'Clock'}
                            </button>
                ...
                <div className="flex-grow flex flex-col justify-center">
                    {viewMode === 'clock' ? (
                      ...
                      <VehicleSection alarmMusicSrc={state.audioSrc} isAlarmPlaying={state.status === 'playing'} />
                    )}
                </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
                <div className="flex justify-between items-center px-1">
                    <label htmlFor="agenda-input" className="text-[10px] font-mono text-radio-dim uppercase tracking-widest">Daily Agenda</label>
                    <input 
                        type="date"
                        className="bg-transparent text-[10px] font-mono text-radio-lit outline-none border-b border-radio-dim/30 focus:border-radio-lit transition-colors"
                        value={state.agendaDate}
                        onChange={(e) => setState(prev => ({ ...prev, agendaDate: e.target.value }))}
                    />
                </div>
      ...
      <audio 
        ref={audioRef} 
        className="hidden" 
        onEnded={() => setState(prev => ({...prev, status: 'idle'}))} 
        onError={() => {
          console.error("Main audio error: Failed to load source");
          setState(prev => ({ ...prev, status: 'error', errorMessage: 'Audio playback failed' }));
        }}
      />
